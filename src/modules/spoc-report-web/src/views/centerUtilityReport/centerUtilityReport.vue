...
<template>
    <div class="centerUtilityReport">
        ...
        >    
            ...
            <Input v-if="tableType==3" v-model="GLR.classroomName" :placeholder="$t('modules_spoc_jw_web_src_views_1v1_newcourse_2165')" />
        </v-search-collapse>
        <journaling
            ref="classTable"
            v-if="defaultShowCol"
            class="mytable"
            :showSummary="tableType!=3"
            :tableName="tableName"
            :tableData="tableData"
...
import valid, { errors, sysUser, sysCommonSql, report, rptArchiveSchedule } from "../../libs/request";
export default {
    ...
    data() {
        return {
            ...
            tableName: [
                {
                    name: this.$t('modules_spoc_report_web_src_views_centerutilityreport_centerutilityreport_4571'),
                    value: "0"
                },
                {
                    name: this.$t('modules_spoc_report_web_src_views_centerutilityreport_centerutilityreport_4572'),
                    value: "1"
                },
                {
                    name: this.$t('modules_spoc_report_web_src_views_centerutilityreport_centerutilityreport_4573'),
                    value: "2"
                },
                {
                    name: this.$t('modules_spoc_report_web_src_views_centerutilityreport_centerutilityreport_4574'),
                    value: "3"
                }
            ],
            selection: [],
            updateShowTitle: report.rptUpdateShowTitle,
            updateShowTitleKey: "RptClassroomArchiveVO",
            defaultShowCol: null,
            sortObj: {},
            tableColumnArray: [],
            tableData: [{}],
    ...
    methods: {
        ...
        exportOk(){
            if(this.exportMonth && !this.exportMonth[0]){
                ...
                report
                .rptUtilityArchiveExportFile(param)
                .then(valid.call(this))
                .then(res => {
                    if (res.ok) {
        ...
        changeTableType(val) {
            ...
            if (this.storageDataObj[this.tableType].data) {
                ...
                if (searchObj) {
                    ...
                    this.GLR.classroomName = searchObj.classroomName
        ...
        getListData() {
            ...
            delete param.searchObjMonth;
            param.startTime = this.GLR.month[0]
                ? new Date(this.GLR.month[0]).format("yyyy-MM") +
                  "-01 00:00:00"
                : "";
            param.endTime = this.GLR.month[1]
                ? getMonthEndDay(
            ...
            let myRequestList;
            if (this.tableType == "0") {
                myRequestList = report.rptClassroomArchiveList(param);
            } else if (this.tableType == "1") {
                 delete param.teacherId;
                 
                param.type = "classTeacher";
                // param.userName = "";
                myRequestList = report.listCollectPage(param);
                ...
                param.orderByType = "lessonDate"
                param.orderByStatus = 2
                param.archiveStatus = this.gdType
                // delete param.userId;
                myRequestList = report.listDetailPage(param);
            }
        ...
        getShowTitle(noSearch) {
            ...
            if (this.tableType == "0") {
                param = {
                    pageIdentifier: "RptClassroomArchiveVO",
                    voName:
                        "com.windliven.spoc.modules.rpt.vo.RptClassroomArchiveVO"
                };
            } else if (this.tableType == "1" || this.tableType == "2") {
                // this.GLR.userId = "";
                param = {
                    pageIdentifier: "RptUtilityArchiveVO",
                    voName:
                        "com.windliven.spoc.modules.rpt.vo.RptUtilityArchiveVO"
                };
            } else if (this.tableType == "3") {
                this.hasPage = true;
                param = {
                    pageIdentifier: "RptUtilityArchiveDetailVO",
                    voName:
                        "com.windliven.spoc.modules.rpt.vo.RptUtilityArchiveDetailVO"
                };
            }
            report
                .rptGetShowTitle(param)
                .then(valid.call(this))
                .then(res => {
                    if (res.ok) {
                        ...
                        if (this.tableType == "1") {
                            ...
                            this.defaultShowCol.true = this.defaultShowCol.true.filter(
                                item => {
                                    return item.name != "classTeacherName";
                                }
                            );
                        }
    ...
    watch: {
        "app.currOfficeId": function(val, oldVal) {
            if (oldVal && this.$route.name == "report.centerUtilityReport") {
                this.pageNo = 1
                this.initOffice();
            }
        }
