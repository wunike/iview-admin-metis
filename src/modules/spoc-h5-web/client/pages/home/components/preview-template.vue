    ...
    <previewWrapper :pageId="pageId" @closePreview="closePreview" v-loading="loading">
        ...
        <div class="preview-info-wrapper" v-if="!loading">
            <el-form label-width="90px" label-position="left">
                <el-form-item label="页面二维码:"><QrcodeVue :value="pageLink" :size="120" level="H"></QrcodeVue></el-form-item>
                <el-form-item label="页面链接:">
                    <div><el-button type="primary" @click="doCopy">复制链接</el-button></div>
                    <div class="share-wx-config-wrapper">{{ $config.baseURL1 + '/page/view/' + pageId }}</div>
                </el-form-item>
                <!--页面效果-->
            ...
            <div class="page-info">
                <div class="page-cover"><img :src="shareData.coverImage || defaultCoverImage" alt="" /></div>
                <div class="page-title-des paddingT10">
                    <div class="info-form-wrapper">{{ shareData.title }}</div>
                    <div class="info-form-wrapper ellipsis">{{ shareData.description }}</div>
                </div>
            </div>
            <div class="clearfix paddingT30 text-center">
                <a :href="pageLink" target="_blank"><el-button type="primary">新标签打开链接</el-button></a>
            </div>
        </div>
    </previewWrapper>
</template>
...
export default {
    ...
    computed: {
        pageLink() {
            return this.$config.baseURL1 + '/page/view/' + this.pageId;
        },
        shareData() {
    ...
    methods: {
        ...
        getData() {
            this.loading = true;
            // this.$axios.get('/api/h5/page/detail/' + this.pageId).then(res => {
            // 	this.loading = false;
            // 	this.pageData = res.body;
            // }).catch(() => {
            // 	this.loading = false;
            // })
            api.pageDetail(this.pageId)
