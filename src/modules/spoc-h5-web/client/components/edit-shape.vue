...
export default {
    ...
    methods: {
        ...
        getPointStyle(point) {
            ...
            const style = {
                marginLeft: hasL ? '-5px' : hasR ? '-5px' : '-5px',
                marginTop: hasT ? '-5px' : hasB ? '-5px' : '-5px',
                left: `${newLeft}px`,
                top: `${newTop}px`,
                cursor:
                    point
        ...
        handleMouseDownOnElement(e) {
            ...
            let move = moveEvent => {
                ...
                // projectData.width
                let showGuide=false;
                if (Math.abs(this.projectData.width/2 - pos.left - width/2) < 11) {
                    showGuide=true
                }
                this.$emit('resize', pos,showGuide);
            };
            let up = () => {
                let showGuide=false;
                if (Math.abs(pos.top) < 11) {
                    pos.top = 0;
                }
                if (Math.abs(this.projectData.height - pos.top - height) < 11) {
                    pos.top = this.projectData.height - height;
                }
                if (Math.abs(pos.left) < 11) {
                    pos.left = 0;
                }
                if (Math.abs(this.projectData.width - pos.left - width) < 11) {
                    pos.left = this.projectData.width - width;
                }
                if (Math.abs(this.projectData.width/2 - pos.left - width/2) < 11) {
                    showGuide=false;
                    pos.left = this.projectData.width/2 - width/2;
                }
