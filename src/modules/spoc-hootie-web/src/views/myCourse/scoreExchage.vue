	...
	<div>
		...
		>
			...
			<div slot="footer">
				...
				<Button type="primary" @click="scoreOK">{{$t('mycourse_scoreexchage_417')}}</Button>
			</div>
		</Modal>
		<exchange ref="exchange" :stuList="stuList" @getfaList="searchInfo"></exchange>
	</div>
</template>

<script>
import exchange from "../classRoom/exchange";
...
export default {
	...
	methods: {
		...
		scoreOK() {
			//                console.log(this.currentRowObj.jxLessonStudentRel)
			if (this.currentRowObj.jxLessonStudentRel == undefined) {
				...
				if (ifPresent == "present") {
					//                        this.showModel=false
					this.$refs.exchange.show("course");
					//                        this.resetSearch()
					//                        this.currentRowObj = {}
				} else {
