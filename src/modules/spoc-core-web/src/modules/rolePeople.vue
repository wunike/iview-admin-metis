...
<template>
	<Modal :title="$t('modules_rolepeople_204')" v-model="rolepeople.visible" class-name="vertical-center-modal" width="730px">
		<div class="body">
			<div class="clearfix">
				<div class="select-box">
					<p class="header">
						<Input class="ss" icon="search" :placeholder="$t('modules_rolepeople_205')" @on-click="onPeopleSearch" @on-enter="onPeopleSearch" v-model="searchForm.text"></Input>
					</p>
					<div class="cbody">
						<div>
							<p class="s-item">{{$t('modules_rolepeople_206')}}</p>
						...
						<div>
							<p class="s-item">{{$t('modules_rolepeople_207')}}</p>
						...
						<div>
							<p class="s-item">{{$t('modules_rolepeople_208')}}</p>
						...
						<div>
							<p class="s-item">{{$t('modules_rolepeople_209')}}</p>
						...
						<div class="btncenter">
							<Button type="primary" @click="doSearch">{{$t('modules_rolepeople_210')}}</Button>
						</div>
					</div>
				</div>
				<div class="waitfor-box">
					<p class="header fr">
						<span class="title">{{$t('modules_rolepeople_211')}}</span>
						<Checkbox v-model="checkleft" @on-change="selectLeft">{{$t('modules_rolepeople_212')}}</Checkbox>
					</p>
					<div class="content">
						<div class="clearfix">
							...
							<template v-else>
								<p>{{$t('modules_rolepeople_213')}}</p>
							</template>
						</div>
					</div>
					<div class="footer">
						<Button @click="toadd">{{$t('modules_rolepeople_214')}}<i class="iconfont icon-youjiantou"></i> </Button>
					</div>
				</div>
				<div class="selected-box">
					<p class="header fr">
						<span class="title">{{$t('modules_rolepeople_215')}}</span>
						<Checkbox v-model="checkright" @on-change="selectRight">{{$t('modules_rolepeople_212')}}</Checkbox>
					</p>
					<div class="content">
						<div class="clearfix">
					...
					<div class="footer">
						<Button @click="toremove"> <i class="iconfont icon-zuojiantou"></i>{{$t('modules_rolepeople_216')}}</Button>
					</div>
				</div>
			</div>
		</div>
		<div slot="footer">
			<Button @click="btnClose">{{$t('modules_rolepeople_217')}}</Button>
			<Button type="primary" @click="btnYes">{{$t('modules_rolepeople_218')}}</Button>
		</div>
	</Modal>
</template>
<script>
	...
	export default {
		...
		methods: {
			...
			searchData(param) {
				...
				sysUser.listUnAssignUserMap(param).then(valid.call(this)).then(res => {
