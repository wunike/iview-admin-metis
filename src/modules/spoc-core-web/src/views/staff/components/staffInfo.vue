    ...
    <Modal
        ...
        <Form ref="form" :rules="ruleCustom"  class="form-box" :model="form" label-position="right" :label-width="120">
            <div class="form-row-line" style="margin-top:24px;">
                <FormItem :label="$t('views_staff_components_staffinfo_668')" prop="no">
                    <Input style="width:250px;" v-model="form.no"></Input>
                </FormItem>
                <FormItem :label="$t('views_staff_components_staffinfo_669')" prop="gender">
                    <RadioGroup style="width:250px;" v-model="form.gender">
                        <Radio :disabled="fordbidAll" value="m" :label="$t('views_staff_components_staffinfo_670')"></Radio>
                        <Radio :disabled="fordbidAll" style="margin-left:20px;"  value="f" :label="$t('views_staff_components_staffinfo_671')"></Radio>
                    </RadioGroup>
                </FormItem>
            </div>
            <div class="form-row-line">
                <FormItem  :label="$t('modules_rolemanage_167')" prop="name">
                    <Input :disabled="fordbidAll" style="width:250px;" v-model="form.name"></Input>
                </FormItem>
                <FormItem :label="$t('memberlist_compontents_modify_241')" prop="enName">
                    <Input :disabled="fordbidAll" style="width:250px;" v-model="form.enName"></Input>
                </FormItem>
            </div>
            <div class="form-row-line">
                <FormItem :label="$t('views_staff_components_staffinfo_674')" prop="mobile">
                    <Input :disabled="fordbidAll" style="width:250px;" v-model="form.mobile" @on-keyup="setUsrName(form.mobile)" ></Input>
                </FormItem>
                <FormItem  :label="$t('modules_rolemanage_169')" prop="email">
                    <Input :disabled="fordbidAll" style="width:250px;" v-model="form.email" ></Input>
                </FormItem>
            </div>
            <div class="form-row-line" >
                <FormItem :label="$t('views_staff_components_staffinfo_676')" prop="loginName">
                    <Input style="width:250px;" disabled v-model="form.loginName"></Input>
                </FormItem>
                <FormItem :label="$t('views_staff_components_staffinfo_677')" prop="userTypeName">
                    <Select :disabled="fordbidAll" v-model="form.userTypeName" style="width:250px">
                        <Option v-for="item in form.userTypeValueList" style="width:250px" :value="item.value" :key="item.value">{{ item.label }}</Option>
                    </Select>
                </FormItem>
            </div>
            <div class="form-row-line">
                <FormItem :label="$t('views_staff_components_staffinfo_678')" prop="officeId">
                    <Select :disabled="fordbidAll" v-model="form.officeId" style="width:250px" @on-change="changeOffice(form.officeId)">
                        <Option v-for="item in form.officeIdList" style="width:250px" :value="item.value" :key="item.value">{{ item.label }}</Option>
                    </Select>
                </FormItem>
                <FormItem :label="$t('views_staff_components_staffinfo_679')" prop="job">
                    <Select :disabled="fordbidAll" v-model="form.job" style="width:250px" @on-change="chooseJob(form.job)">
                        <Option v-for="item in form.jobValueList" style="width:250px" :value="item.value" :key="item.value">{{ item.label }}</Option>
                    </Select>
                </FormItem>
            </div>
            <div class="form-row-line">
                <FormItem :label="$t('views_staff_components_staffinfo_680')" prop="departIds">
                    <Select :disabled="fordbidAll" v-model="form.departIds" :placeholder="$t('views_staff_components_staffinfo_681')" multiple  style="width:250px" >
                        <Option
                            v-for="item in form.departidsList"
                            style="width:250px"
                            :value="item.value"
                            :key="item.value"
                            :disabled="item.value === form.officeId"
                        >{{ item.label }}</Option>
                    </Select>
                </FormItem>
                <FormItem :label="$t('views_staff_components_staffinfo_682')" prop="roleIds" v-if="showModel">
                    <Select :disabled="fordbidAll" v-model="form.roleIds"
                            :placeholder="$t('views_staff_components_staffinfo_683')" multiple style="width:250px">
            ...
            <div class="form-row-line">
                <FormItem :label="$t('views_staff_components_staffinfo_684')" prop="isService">
                    <RadioGroup  style="width:250px;" v-model="form.isService">
                        <Radio :disabled="this.status()"  value="1" :label="$t('views_staff_components_staffinfo_685')"></Radio>
                        <Radio :disabled="this.LeaveStatus()" style="margin-left:20px;" value="0" :label="$t('views_staff_components_staffinfo_686')"></Radio>
                    </RadioGroup>
                </FormItem>
                <FormItem :label="$t('views_staff_components_staffinfo_687')" prop="crossIds">
                    <Select :disabled="fordbidAll" v-model="form.crossIds" :placeholder="$t('views_staff_components_staffinfo_681')" multiple  style="width:250px">
                        <Option v-for="item in form.crossIdsList"
                                :disabled="item.value === form.officeId"
                                style="width:250px"
                                :value="item.value"
            ...
            <div class="form-row-line">
                <FormItem  :label="$t('views_staff_components_staffinfo_688')" prop="entryDate">
                    <DatePicker :disabled="fordbidAll" v-model="form.entryDate"  type="date" :placeholder="$t('scoretemplate_compontents_scoremodify_528')" style="width: 250px"></DatePicker>
                </FormItem>
                <FormItem :label="$t('views_staff_components_staffinfo_690')" prop="leaveDate">
                    <DatePicker :disabled="this.LeaveStatus()" v-model="form.leaveDate" type="date" :placeholder="$t('scoretemplate_compontents_scoremodify_528')" style="width: 250px"></DatePicker>
                </FormItem>
            </div>
            <div class="form-row-line" :class="showTeacher ? 'form-row-lines1' : 'form-row-lines2'">
                <div style="margin-left:48px;font-size:14px;font-weight:bold;cursor:pointer;" @click="changeShowSenior">{{$t('views_staff_components_staffinfo_691')}}<Icon :type="showSenior ?'ios-arrow-up':'ios-arrow-down'" /><span style="margin-left:12px;color:#CCC;">{{$t('views_staff_components_staffinfo_692')}}</span>
                </div>
            </div>
            <div v-show="showSenior" class="form-row-line" :class="showTeacher ? 'form-row-lines1' : 'form-row-lines2'">
                <FormItem :label="$t('views_staff_components_staffinfo_693')" prop="adminIds">
                    ...
                    <Checkbox :disabled="fordbidAll" v-model="form.selectAll" @on-change="xqIdsChangeAll">{{$t('modules_rolepeople_212')}}</Checkbox>
                </FormItem>
            </div>
            ...
            <div v-if="showTeacher" class="form-row-line" style="margin-bottom:24px;">
                <FormItem  :label="$t('views_staff_components_staffinfo_696')" prop="teacherType">
                    <Select :disabled="teacherFb ? !teacherFb : fordbidAll" v-model="form.teacherType" style="width:224px;">
                ...
                <FormItem :label="$t('views_staff_components_staffinfo_697')" prop="teachableDuration">
                    <Input :disabled="teacherFb ? !teacherFb : fordbidAll" style="width:200px;" v-model="form.teachableDuration"></Input>
                    <span>{{$t('views_staff_components_staffinfo_698')}}</span>
                </FormItem>
            </div>
            <div slot="footer">
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                showModel:false,
                showTeacher:false,
                reWriteAgain:false, //禁止no
                fordbidAll:true,//离职禁止
                teacherFb:false, // 老师修改时 禁止
                staffId:'',
                formState:'',
                loading:true,
                form:{
                ...
                ruleCustom:{
                    ...
                    loginName:[
                        {required:true,message:this.$t('pushsettings_index_504')}
                    ],
                    userTypeName:[
                    ...
                    // job:[
                    //     { required:true, validator: jobValuePass, trigger: 'change' }
                    // ],
                    roleIds:[
                        {required:true, type: 'array', min: 1, message:this.$t('pushsettings_index_507'),  trigger: 'change' }
                    ],
                    mobile: [
                        {required:true, validator: number,  trigger: 'blur' }
                    ],
                    isService: [
                        {required:true, validator: isServesPass,  trigger: 'change' }
                    ],
                    entryDate: [
                        {required:true, validator: validatePass,  trigger: 'blur' }
                    ],
                    email: [
                        { type: 'email', message:this.$t('views_staff_components_staffinfo_705'),trigger: 'blur' },
                    ],
                    teacherType:[
                        {required:true, validator: validatePass, trigger: 'change' }
                    ],
                    teachableDuration:[
                        // {required:true, validator: teachableDurations,trigger: 'blur' }
                    ],
                },
            }
        },
        ...
        methods: {
            ...
            status() {
                if(this.fordbidAll && this.teacherFb){ //老师
                    return true
                }else if( this.fordbidAll && !this.teacherFb){ //离职
                    return false
                }else if( !this.fordbidAll && !this.teacherFb){
                    return false
                }
            },
            ...
            forbid(val){//禁止修改表单
                if(val === this.$t('views_staff_components_staffinfo_686')){
                    this.fordbidAll= true
                    this.teacherFb = false
                }else if(val === this.$t('views_staff_components_staffinfo_706') ){
                    this.fordbidAll= true
                    this.teacherFb = true
                } else{
                    this.teacherFb = false
                    if(val === this.$t('views_staff_components_staffinfo_707')){
                        this.reWriteAgain= true
                    }else {
                        this.reWriteAgain= false
                    }
                    this.fordbidAll= false
                }
            },
            changeOffice(value) {
            ...
            getFormList(){
                ...
                sysOffice.officeListData().then(valid.call(this)).then((res)=>{
                    if(res.ok){
                        let data = res.data.data.allOffice
                ...
                sysRole.myRoleList().then(valid.call(this)).then((res)=>{
                    if(res.ok){
                ...
                if(row){
                    if(row.id){
                        if(type === 1){// 员工修改
                            ...
                            this.handleReset('form')
                            this.form.roleIds = [];
                            this.formState = this.$t('views_staff_components_staffinfo_686')
                            this.leaveStaff(row)
                            this.comSetModel(row)
                        }else{ //教师修改
                            this.handleReset('form')
            ...
            comSetModel(row){//显示控制
                this.staffId = row.id
                this.showTeacher =   this.form.job === 'teacher' ? true : false
            },
            leaveStaff(row) {
            ...
            conmonStaff(row){//公共函数
                this.form.no = row.no
                this.form.gender = row.gender=== 'f' ? this.$t('views_staff_components_staffinfo_671') : row.gender=== 'm' ? this.$t('views_staff_components_staffinfo_670') :''
                ...
                this.form.isService = row.isService=== '0' ? this.$t('views_staff_components_staffinfo_686') : row.isService=== '1' ? this.$t('views_staff_components_staffinfo_685') :''
...
//                console.log(this.form)
                this.$refs[name].validate((valid1) => {
                    if (valid1) {
                        ...
                        params.gender = this.form.gender === this.$t('views_staff_components_staffinfo_671') ? 'f' : this.form.gender === this.$t('views_staff_components_staffinfo_670') ? 'm':''
                        ...
                        params.isService = this.form.isService === this.$t('views_staff_components_staffinfo_685') ? '1' :this.form.isService === this.$t('views_staff_components_staffinfo_686') ? '0':''
                        ...
                        if(type === 'add'){
                            ...
                            sysUser.saveUser(params).then(valid.call(this)).then((res)=>{
                                if(res.ok){
                            ...
                            if(this.form.isService === this.$t('views_staff_components_staffinfo_686')){
                                if(params.entryDate && params.leaveDate){
                                    ...
                                    if(date2 >= date1 ){
                                        ...
                                        this.isSaveUsering = true
                                        sysUser.updateIsService(params).then(valid.call(this)).then((res)=>{
                                            if(res.ok){
