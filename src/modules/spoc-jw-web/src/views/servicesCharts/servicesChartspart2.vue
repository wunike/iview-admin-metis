...
<template>
    <div class="servicesChartspart2">
        <div class="count">
            ...
            <span>{{$t('modules_spoc_jw_web_src_views_servicescharts_serviceschartspart2_3086', [totalAmount])}}</span>
            <span style="margin-left:24px;">{{$t('modules_spoc_jw_web_src_views_servicescharts_serviceschartspart2_3087', [average])}}</span>
        </div>
            ...
            .then(res => {
                if (res.ok) {
                    ...
                    result.servicePerformance.forEach((item)=>{
                        if(teacherNameArray.length < 10){  //取前十名
                            ...
                            item.courseTypeStatisticals.forEach((childrenItem)=>{
                                teacherNameDataObj[item.name][childrenItem.name] = childrenItem.consumeAmount
                            })
                        }
                    })
                    ...
                    let option1 = barOption1({
                        ...
                        teacherNameArray,
                        allTypeArrObj,
                        yAxisName: this.$t('modules_spoc_crm_web_src_views_activityenrollment_components_createevent_617')
                    })
