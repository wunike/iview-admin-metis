...
<template>
    <div class="servicesChartspart3">
        <div class="count">
            ...
            <span>{{$t('modules_spoc_jw_web_src_views_servicescharts_serviceschartspart3_3091', [totalAmount])}}</span>
            <span style="margin-left:24px;">{{$t('modules_spoc_jw_web_src_views_servicescharts_serviceschartspart3_3092', [average])}}</span>
        </div>
            ...
            .then(res => {
                if (res.ok) {
                    ...
                    result.servicePerformance.forEach((item)=>{
                        if(teacherNameArray.length < 10){  //取前十名
                            ...
                            item.courseTypeStatisticals.forEach((childrenItem)=>{
                                teacherNameDataObj[item.name][childrenItem.name] = childrenItem.consumeHour
                            })
                        }
                    })
                    ...
                    let option1 = barOption1({
                        ...
                        teacherNameArray,
                        allTypeArrObj,
                        yAxisName: this.$t('views_coursepack_coursepack_379')
                    })
