...
<template>
	<div class="chargingManageList">
		<v-search-collapse @search="doSearch" @reset="resetSearch" @changeDivHeight="changeDivHeight">
			...
			<!-- <Input v-model="value" :placeholder="$t('modules_spoc_jw_web_src_views_chargingmanage_chargingmanagelist_2339')" style="width: 224px" /> -->
		</v-search-collapse>
		<big-table
			v-if="defaultShowCol"
			class="mytable"
			...
			updateShowTitleKey="listPageDeducted"
			:dataTotal="dataTotal"
			:pageNo="pageNo"
		...
		>
			<div slot="tableNameSlot">{{$t('modules_spoc_jw_web_src_views_chargingmanage_chargingmanagelist_2340')}}<span class="tableNamespan">{{totalNum || 0}}</span>{{$t('modules_spoc_jw_web_src_views_chargingmanage_chargingmanagelist_2341')}}<span class="tableNamespan">{{totalDeducted || 0}}</span>{{$t('modules_spoc_crm_web_src_views_activityenrollment_components_createevent_617')}}</div>
		</big-table>
...
export default {
	...
	data() {
		return {
			...
			exportMethod: jwCourseConsumeLog.export, //导出用到的方法对象
			defaultShowCol: null,
			dataTotal: 0,
			sortObj: {},
			tableColumnArray: [
	...
	methods: {
		...
		resetDefaultCol(){
            ...
            jwCourseConsumeLog.clearShowField({
                pageIdentifier: "listPageDeducted",
            })
			...
			jwCourseConsumeLog
				...
				.then(res => {
					if (res.ok) {
						...
						//{totalNum: "10", totalDeducted: "4197.5836"}
						this.totalNum = result.totalNum;
						this.totalDeducted = result.totalDeducted;
					}
				})
				.catch(errors.call(this))
				.finally(() => {
		...
		getListData() {
			...
			let param = {
				...
				deductStartDate: this.operateTime[0] ? new Date(this.operateTime[0]).format('yyyy-MM-dd 00:00:00') : '',
				deductEndDate: this.operateTime[1] ? new Date(this.operateTime[1]).format('yyyy-MM-dd 23:59:59') : '',
				officeId: this.officeId,
				operateName: this.operater,
			...
			this.searchObj = Object.assign({},param); //这里需要给searchObj 赋值，导出数据的时候需要用到
			// console.log(param)

			jwCourseConsumeLog
				.listPageDeducted(param)
				.then(valid.call(this))
				.then(res => {
					if (res.ok) {
						let result = res.data.data;
	...
	watch: {
		"app.currOfficeId": function(val, oldVal) {
			if (oldVal && this.$route.name == 'jw.chargingManageList') {
				this.officeId = this.app.currOfficeId == 'all' ? '' : this.app.currOfficeId;
				this.getListData();
				this.changeSchool(this.officeId)
			}
