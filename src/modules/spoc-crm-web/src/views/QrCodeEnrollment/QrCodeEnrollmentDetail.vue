    ...
    <div class="QrCodeEnrollmentDetail">
        <div class="QrCodeEnrollmentDetailTop">
            ...
            <div class="detailInfo">
                <div class="detailInfoTop" v-if="formObj.tags">
                    <div class="spanLabelTitle">{{$t('modules_spoc_crm_web_src_views_qrcodeenrollment_qrcodeenrollmentdetail_1576')}}</div>
                    <div class="spanLabel">
                        <!-- <p v-if="!formObj.tags"></p> -->
                        <span v-for="item in formObj.tags.split(' ')" :key="item">{{item}}</span>
                    </div>
                </div>
                <div class="detailInfoBottom">
                    <div>
                        <div>{{$t('modules_spoc_crm_web_src_views_qrcodeenrollment_qrcodeenrollmentdetail_1577')}}<span
                            ...
                            <!-- <Icon type="ios-browsers-outline" class="QrNoClass" /> -->
                        ...
                        <div>{{$t('modules_spoc_crm_web_src_views_qrcodeenrollment_qrcodeenrollmentdetail_1579')}}<span>{{formObj.detailDate}}</span>
                        </div>
                        <div>{{$t('modules_spoc_crm_web_src_views_qrcodeenrollment_qrcodeenrollmentdetail_1580')}}<span>{{formObj.createByName}}</span>
                        </div>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div>{{$t('modules_spoc_crm_web_src_views_activityenrollment_components_entrydetial_712')}}<span>{{formObj.source}}</span>
                        </div>
        ...
        <div class="QrCodeEnrollmentDetailButtom">
            ...
            <div>{{$t('modules_spoc_crm_web_src_views_activityenrollment_components_detial_660')}}<span>{{formObj.visits}}</span>
            </div>
            <div>
                Leads：
                <span>{{formObj.leads}}</span>
            </div>
            <div>
                Vleads：
                <span>{{formObj.vleads}}</span>
            </div>
            <div>
                APP：
                <span>{{formObj.app}}</span>
            </div>
            <div>
                OPP：
                <span>{{formObj.opp}}</span>
            </div>
            <div>{{$t('modules_spoc_crm_web_src_views_activityenrollment_components_detial_661')}}<span>{{formObj.anew}}</span>
            </div>
        </div>
        <div class="QrCodeEnrollmentDetailButtom">
            <div style="width:24%;">{{$t('modules_spoc_crm_web_src_views_activityenrollment_components_detial_664')}}<span
                    style="color:#44BCB7"
                >{{!isNaN(formObj.appPercent)?formObj.appPercent*100+'%':''}}</span>
            </div>
            <div style="width:19%;">{{$t('modules_spoc_crm_web_src_views_qrcodeenrollment_qrcodeenrollmentdetail_1591')}}<span
                    style="color:#44BCB7"
                >{{!isNaN(formObj.oppPercent)?formObj.oppPercent*100+'%':''}}</span>
            </div>
            <div style="width:19%;">{{$t('modules_spoc_crm_web_src_views_qrcodeenrollment_qrcodeenrollmentdetail_1592')}}<span
                    style="color:#44BCB7"
                >{{!isNaN(formObj.newPercent)?formObj.newPercent*100+'%':''}}</span>
            </div>
        </div>
        <big-table
            v-if="defaultShowCol"
            ...
            <div v-if="titleVal==5">
                ...
                <div style="width:100%;display:flex;justify-content:center;align-items:center">
                    <div
                        class="posterView"
                        id="posterViewdom"
                        :style="`width:750px;height:1344px;display:flex;justify-content:center;align-items: center;background-image: url(${posterData.posterUrl});background-size: cover;background-repeat: no-repeat;`"
                    >
...
export default {
    ...
    computed: {
        ...
        posterQRCodeLeft:function(){
            ...
            if(this.posterData.posterQrCodePointXy.x){
                return this.posterData.posterQrCodePointXy.x/320 *100 +0.6+'%';
            }else{
                return 220*ratio+'px';
            }
        },
        posterQRCodeTop:function(){
            ...
            if(this.posterData.posterQrCodePointXy.y){
                return this.posterData.posterQrCodePointXy.y/568 *100 +'%';
            }else{
                return 480*ratio+'px';
            }
        },
    ...
    methods: {
                        ...
                        this.$nextTick(() => {
                            ...
                            ele.forEach(v=>{
                                ...
                                this.showQRCode(
                                    ...
                                    window.location.origin +
                                        "/#/h5Form.html?id=" +
                                        this.rowTableObj.qrId +
                                        "&channelId=" +
                                        this.rowTableObj.channelId +
                                        "&tenant=" + (this.tenant || localStorage.getItem('tenant'))
                                );
                            })
                        });
        ...
        getListData() {
            ...
            let myRequestList;
            if (this.tableType == "1") {
                this.defaultShowCol = this.cols1;
                myRequestList = crmQrcode.findQrcodeChannel;
            } else if (this.tableType == "2") {
                this.defaultShowCol = this.cols2;
                myRequestList = crmQrcode.findQrcodeCustomer;
            } else if (this.tableType == "3") {
                this.defaultShowCol = this.cols3;
                myRequestList = crmQrcode.findQrcodeMember;
            }
        ...
        downloadposter(){
            ...
            .then(function (blob) {
                window.saveAs(blob, _this.rowTableObj.name+(_this.rowTableObj.phone? _this.rowTableObj.phone:'')+'.png');
            });
        }
    }
};
