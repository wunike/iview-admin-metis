<style lang="less">
.course-time-lists-container-crm {
...
<template>
	<div class="full-height-sign">
		...
		<div
			class="course-time-lists-container-crm"
			v-else
			...
			<div class="time-table-bottom full-height-sign-bottom" >
				<table-header
				...
				<div
					class="table-body full-height-sign-body"
					:class="{'shrink-table': !showTableDetail}"
					id="tableBody">
						...
						<ul class="flex-ul">
							<li v-for="(card, index1) in 7" :key="'tabel-li-data' + card">
								<div
									...
									:key="index2"
                                    :class="{
                                        bgColor0: item.lessonStatus == 'unAttendClass', 
                                        bgColorA: item.lessonStatus == 'unAttendance', 
                                        bgColorB: item.lessonStatus == 'unFinishAttendance', 
                                        bgColorC: item.lessonStatus == 'finishAttendance', 
                                        bgColorD: item.lessonStatus == 'unNormal',
									...
									<div class="time-table-detail" @click="clickCard(item)">
										<div v-if="showTableDetail">
											<div v-if="(item.height >= 1.5)">
												<card-detail
													:date="date"
													:item="item"
													:headerLists="headerLists"
											...
											<Tooltip :placement="item.start * rowHeight < 45 * 4 ? 'bottom' : 'top'" v-else>
												<!-- <Tooltip :placement="item.start * rowHeight < 45 * 4 ? 'bottom' : 'top'"> -->
									...
									<div
										...
										<ul>
											<li v-for="(student, index) in item.detail.jwStudentLessonAttendanceVOList" :key="index">
												...
												<div class="fr">
													<span style="margin-right: 4px;">{{ student.lessonStatus }}</span>
...
export default {
	...
	methods: {
		initListsSignIn() {
			...
			if (this.lists && this.lists.length) {
				...
				this.lists.forEach(e => {
                    ...
                    if (e.lessonStatus == "unNormal") {
                        // 冲突
                        _lists.timeCard[e.week - 1].lists.push(
                            this.formatCard(
                                e,
                                __startTimeHourZero,
                                _sbnormalNum
		...
		formatCard(_item, startTimeHourZero, _sbnormalNum) {
			// 将获取的数据格式化为方块需要的形式
			// console.log(startTimeHourZero)
			...
			let obj = {
				lessonStatus: _item.lessonStatus,
				start: start,
				height: height,
				right: `${right}px`,
				left: `${left}px`,
