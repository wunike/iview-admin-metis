...
<template>
	<div class="wechatUser">
		...
		>
			<Input v-model="record.name" clearable placeholder="微信号或昵称" />
			<DatePicker
				ref="DatePicker"
				v-model="optTime"
				type="daterange"
			...
			>
				<Option
					v-if="roleOptions2 && roleOptions2.length"
			...
			>
				<Option
					v-if="areaSchoolList && areaSchoolList.length"
			...
			>
				<Option
					v-if="roleOptions4 && roleOptions4.length"
					v-for="(item, index) in roleOptions4"
					:value="item.value"
					:key="index"
					>{{ item.name }}</Option
				>
		...
		></assignLeadsBySingle>
		<Modal v-model="showDetailModal" title="客户详情">
			<div class="showDetailModal">
				<div class="wxInfos">
					<div class="wxInfo">
						<img
						...
						<div class="nameAndTag">
							...
							<div class="userTargetInner">
								<span
									v-if="userLabelData && userLabelData.length"
									v-for="item in userLabelData"
									:key="item"
									>{{ item }}</span
								>
								<userLabel
									...
									:flag="0"
									:echoDisplay="true"
									:defaultCheckedTagids="tags"
									@userLabelTrueChoose="userLabelTrueChoose1"
								>
					...
					<div style="width: 60px">微信联系人</div>
				</div>
				<div class="wxDetail">
					<p><span>添加人：</span>抖落半抹微凉</p>
					<p><span>所在部门：</span>集团总部</p>
					<p><span>添加时间：</span>2020-10-20 16:00</p>
				</div>
			</div>
			<div slot="footer">
				<Button type="primary" @click="showDetailModal = false"
					>关闭</Button
				>
			</div>
		</Modal>
		<Modal v-model="msgRecordModal" width="800" title="聊天记录">
			<div class="msgRecordModal">
				<div class="lxr">
					<div class="lxrTitle">联系人</div>
					<div class="lxrInfos">
						<div class="lxrInfo active">
							<img
			...
			<div slot="footer">
				<Button type="primary" @click="msgRecordModal = false"
					>关闭</Button
				>
			</div>
		</Modal>
...
import assignLeads from "../../customerManagement/assignLeads";
...
export default {
	...
	data() {
		return {
			...
			updateShowTitleKey: "crm.customerManagementaaa",
						...
						return h(
							...
							[
								...
								h(
												...
												if (
													...
													this.$Modal.confirm({
														...
														onOk: () => {
															...
															this.$nextTick(
																() => {
																	this.$refs.assignLeadsBySingle.show(
																		params
																			.row
																			.id,
																		true
									...
									this.$t(
										"modules_spoc_crm_web_src_views_customermanagement_index_1287"
									)
								),
								h(
									"a",
				...
				[
					{
						hidden:
							this.myButtonPrem &&
							this.myButtonPrem.indexOf("export") >= 0
								? false
								: true,
	...
	methods: {
		...
		showMsgRecordModal(detailObj) {
			this.msgRecordModal = true;
		},
		userLabelTrueChoose1(data) {
			this.userLabelData = data.hasCheckedTagNames;
			this.tags = data.hasCheckedTagids.join(",");
		},
		showDetail(row) {
		...
		assignLeads() {
			if (this.tableSelected && !this.tableSelected.length) {
				this.$Message.warning(
					this.$t(
						"modules_spoc_crm_web_src_views_customermanagement_index_1310"
					)
				);
				return;
			}
