    ...
    <div class="histroy-lists-container">
        ...
        >
            <div v-if="historyLists && historyLists.length">
                ...
                <div v-if="historyType == '4'">
                    <div class="group-for-month" v-for="(item, index) in historyLists" :key="'img' + index">
                        <div class="date-content" v-if="historyLists.length > 1">
                            <span v-if="item.month == currMonth">今天</span>
                            <span v-else>{{ item.month }}</span>
                        </div>
                        <div class="img-lists">
                            <a :href="h.filePath" v-for="(h, i) in item.lists" :key="i" download><img :src="h.filePath" /></a>
                        </div>
                    </div>
                </div>
                ...
                <div v-else>
                    <div class="group-for-date" v-for="(item, index) in historyLists" :key="index">
                        <div class="date-content" v-if="historyLists.length > 1">
                            <span v-if="item.date == today">今天</span>
                            <span v-else-if="item.date == yesterday">昨天</span>
                        ...
                        <div class="history-item" v-for="(h, i) in item.lists" :key="i">
                            ...
                            <div class="txt" v-else-if="h.msgType == '4'">
                                <!-- 图片 -->
                                <a :href="h.filePath" download><img :src="h.filePath" /></a>
                            </div>
                            ...
                            <div class="txt" v-else-if="h.msgType == '5'">
                                <!-- 语音 -->
                                <audio :src="h.filePath" controls>您的浏览器不支持 audio 标签。</audio>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="no-history" v-else>
                ...
                <p v-if="historyType == '3' || historyType == '4'">没有此类型的聊天内容～</p>
                <p v-else>暂无聊天记录～</p>
            </div>
        </Scroll>
    </div>
...
export default {
    ...
    data() {
        return {
            ...
            yesterday: new Date(new Date().getTime() - 86400000).format('yyyy-MM-dd'),
            pushFlag: true,
            historyType: '', //4：图片，3：文件，''：全部
            currMonth: new Date().format('yyyy年MM月'),
            dialogId: ''
        };
    },
    mounted() {
        ...
        let testobj = {
            list: [
                {
                    id: '18597ba8-3b7f-4899-bc26-c18922005999',
                    dialogId: '6bc30e58690142beaa125b4e26b34ce0',
                    fromId: '18c843a1b0c9e2602f8ac68524041383',
                    fromName: '孙沁而',
                    fromEnName: 'Isablle',
                    fromAvator: 'https://hootie.athenaca.com/spoc-hootie/avator/default.png',
                    fromDate: 1601118688915,
                    fromDateStr: '2020-09-26 19:11:28',
                    toId: '6ba83b2244064c25935100759a6becd2',
                    toName: 'Craig',
                    ...
                    act: 'say',
                    message:
                        '<div style="width:100%;display: flex;align-items: center;word-wrap:break-word;word-break:break-all;white-space:pre-wrap;flex-flow: wrap;" class="emj">Hi Craig, Isabelle forgot bringing her homework sheet back home. Do you have the soft copy in hands?</div>',
                    msgType: '1',
                    filePath: null,
                    isRead: '1',
                    isRevoke: '0',
                ...
                {
                    id: '658a939c-1561-43f1-bae0-e530dfd74bd5',
                    dialogId: '6bc30e58690142beaa125b4e26b34ce0',
                    fromId: '6ba83b2244064c25935100759a6becd2',
                    fromName: 'Craig',
                    fromEnName: 'Craig',
                    fromAvator: 'https://hootie.athenaca.com/spoc-common/avator/staff.png',
                    fromDate: 1606638922983,
                    fromDateStr: '2020-11-29 16:35:22',
                    toId: '18c843a1b0c9e2602f8ac68524041383',
                    toName: '孙沁而',
                    classCourseId: '84e1fc04cbf3453899a00359ce309b92',
                    classCourseName: 'NGR2007',
                    officeId: '13',
                    ...
                    toAvator: null,
                    act: 'say',
                    message: "<p>Hi, I'm sorry. I've only just seen this message.I can give her another copy on Wednesday when she comes to class.</p>",
                    msgType: '1',
                    filePath: null,
                    isRead: '1',
                    isRevoke: '0',
        ...
        this.showSearchChatRecordLists(testobj);
    },
    methods: {
        ...
        findHistory() {
            ...
            if (this.dateRange && this.dateRange.length == 2) {
                params.startTime = new Date(this.dateRange[0] + ' 00:00:00').getTime();
                params.endTime = new Date(this.dateRange[1] + ' 23:59:59').getTime();
        ...
        setListsImg(_data) {
            ...
            _data.list.forEach(history => {
                let currentMonth = new Date(history.fromDateStr).format('yyyy年MM月');
                // console.log(currentMonth)
                // console.log(this.historyLists)
                // 判断已存在的历史数据里面，有没有当前日期的数据
                if (this.historyLists.length && this.historyLists[0].month == currentMonth) {
            ...
            if (arrHistoryLists.length) {
                this.historyLists[0].lists = [...arrHistoryLists, ...this.historyLists[0].lists];
            }
        ...
        setLists(_data) {
            ...
            _data.list.forEach(history => {
                ...
                // console.log(this.historyLists)
                // 判断已存在的历史数据里面，有没有当前日期的数据
                if (this.historyLists.length && this.historyLists[0].date == currentDate) {
                    // 当前日期 和 已存在的list最后一条日期一致
                    arrHistoryLists.push(history);
                } else {
