...
<template>
	<div class="taskCenter">
		<Modal
			...
			:title="$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1771')"
			width="500"
		...
		>
			<div>
				 <Form ref="taskCompleteFrom" :model="taskCompleteObj" :label-width="120" v-show="canShow">
                    <FormItem v-if="detailObj.taskTpl.completeResultList" :label="$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1772')" prop="resultValue"  :rules="{required: true, message: $t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1773'), trigger: 'change'}">
                        <Select v-model="taskCompleteObj.resultValue" class="w306" :placeholder="$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1774')">
                    ...
                    <FormItem :label="$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1775')" prop="remarks"
						:rules="{ required: true, message: $t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1776'), trigger: 'blur' }">
                        <Input v-model="taskCompleteObj.remarks" type="textarea" class="w306"  :autosize="{minRows: 2,maxRows: 5}" :placeholder="$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1777')"></Input>
                    </FormItem>
                </Form>
			</div>
			<div slot="footer">
				<Button @click="cancelTaskComplete" style="margin-right: 16px">{{$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1778')}}</Button>
				<Button type="primary" @click="handleSubmit">{{$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1779')}}</Button>
			</div>
		</Modal>
		<div class="search-title">
			...
			<v-search-collapse @search="doSearch" @reset="resetSearch">
                <Select v-model="searchObj.officeId" style="width: 206px;" clearable :placeholder="$t('modules_spoc_crm_web_src_modules_resourceentrymodal copy_537')">
                ...
                <DatePicker
					...
					separator=" ~ "
					:placeholder="$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1784')"
					style="width:206px;"
				></DatePicker>
				<Select v-model="searchObj.dateTime_status" @on-change="change_dateTime_status" style="width: 140px;" clearable :placeholder="$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1785')">
				...
				<!-- <Input v-model="searchObj.tplName" :placeholder="$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1792')"></Input> -->
				<Select
					v-model="searchObj.tplName"
					:placeholder="$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1793')"
					clearable
                >
					<Option
				...
				},
				{
					...
					render: (h, params) => {
						...
						if(params.row.isSelf == '0'){
							label = this.$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1799')
						}
						return h("div", {}, label)
					}
				},
					...
					render: (h, params) => {
						...
						return h("div", {}, [
							...
							h(
								...
								this.$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1807')
							),
	...
	methods: {
		...
		toCompleteMore(){
			if(!this.selection.length){
				this.$Message.error(this.$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1811'))
				return false
			}
			let taskType = ''
			...
			if(taskTypeFlag){ // 不是同一主题的模板
				this.$Message.error(this.$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1812'))
				return false
			}
			...
			if(taskStatusFlag){ // 存在已完成的任务
				this.$Message.error(this.$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1813'))
				return false
			}
					...
					if(_formData && _formData.length){
						...
						if(formData && formData.length){
							...
							if(uploadFormData.length){
								this.updateLoadingStatus({ isLoading: false });
								this.$Message.error(this.$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1814'))
		...
		handleSubmit(){
			...
			if(this.hasFormData){
				...
				$f.submit((formData,api)=>{
					console.log(this.$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1815') , formData)
					this.doTaskCompleteMore(JSON.stringify(formData))
				},(api)=>{
					console.log(this.$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1816'))
		...
		doTaskCompleteMore(formData){
			this.$refs.taskCompleteFrom.validate((valid) => {
				if (valid) {
					this.taskCompleteModal = false
					let detailList = [{ "content":this.$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1817') }]
		...
		deleteById(id){
			...
			common
				...
				.then(res => {
					if (res.ok) {
						this.$Message.success(this.$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1818'))
						this.doSearch()
					}
			})
			.catch(errors.call(this))
		...
		change_dateTime_status(val){
			if(!val){
                ...
                if(val == '2'){
                    ...
                    // start = new Date(time).format('yyyy-MM-dd 00:00:00')
                    end = new Date(time).format('yyyy-MM-dd 23:59:59')
                }else if(val == '7'){
		...
		getListData(isFirst) {
            ...
            if(this.rule1.length){
                $f.submit(
                    ...
                    api => {
                        //验证未通过
                        console.log(this.$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1820'));
                    }
                );
            }
			let param = {
			...
			this.searchObj.startTimeBegin = this.searchObj.dateTime[0]
				? new Date(this.searchObj.dateTime[0]).format('yyyy-MM-dd 00:00:00')
				: "";
			this.searchObj.startTimeEnd = this.searchObj.dateTime[1]
            ...
            param.endTimeBegin = this.endTimeArr.length ? this.endTimeArr[0] : ''
            param.endTimeEnd = this.endTimeArr.length ? this.endTimeArr[1] : ''
            delete param.dateTime;
            delete param.dateTime_status;
					...
					this.btnList = [
						{
							...
							text: this.$t('modules_spoc_crm_web_src_views_taskcenter_taskcenterall_1821')
						}
					]
				} else {
					this.btnList = [
