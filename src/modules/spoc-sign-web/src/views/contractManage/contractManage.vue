...
<template>
    <div class="contractManage" :style="{ 'padding-top': (ctStatus == 'reject' || (ctStatus != 'reject' && contractCount == 0 ))? '16px' : '' }">
        ...
        <div class="collapse">
            <v-search-collapse @search="doSearch" @reset="resetSearch">
                ...
                <Select v-model="studentId"
                    ...
                    @on-open-change="studentOpenChange"
                    >
                ...
                <Select v-model="category" clearable :placeholder="$t('modules_spoc_report_web_src_views_refundreport_index_4663')" style="width:140px">
                ...
                <Select v-model="receiptAudit" clearable :placeholder="$t('sksp')" style="width:140px">
                    <Option value="0">{{$t('mycourse_mycourse_387')}}</Option>
                    <Option value="1">{{$t('mycourse_mycourse_3871')}}</Option>
                </Select>
            </v-search-collapse>
        </div>
        <div class="cell_wrap">
            <div class="cell_box">
                <Tabs v-model="ctStatus" @on-click="changeFilter"><TabPane :label="$t('message_index_287')" name=""></TabPane><TabPane v-for="item in ctStatusList" :key="item.value" :label="item.label" :name="item.value"></TabPane></Tabs>
            </div>
        </div>
        <div class="contractManageTable">
            ...
            >
                <p slot="tableNameSlot" class="tableName">
                    {{$t('modules_spoc_crm_web_src_views_customer360_components_flowup_796')}}<a>{{ pactInfo.total }}</a>
                    {{$t('modules_spoc_sign_web_src_views_contractmanage_contractmanage_5686')}}<a style="color:#F5222D;">{{ pactInfo.signPriceTotal ? (Number(pactInfo.signPriceTotal)/10000).toFixed(2) : '-' }}</a>
                    {{$t('modules_spoc_crm_web_src_views_performanceplan_modal_1482')}}
                    <!-- 全部、履行中、已结束、已退款 -->
                    <span v-show="ctStatus==0 || ctStatus=='normal' || ctStatus=='close' || ctStatus=='refund'">{{$t('modules_spoc_sign_web_src_views_contractmanage_contractmanage_56861')}}<a style="color:#F5222D;">{{pactInfo.signPriceTotal ? (Number(pactInfo.remainPrice)/10000).toFixed(2) : '-'}}</a>{{$t('modules_spoc_crm_web_src_views_performanceplan_modal_1482')}}</span>
                </p>
            </big-table>
        </div>
...
import valid, { errors, htContract, common, comAuditFlow, sysDict, sysUser, sysCommonSql, sysAttachment, api } from '../../libs/request';
...
export default {
    ...
    computed: {
        ...
        myButtonPrem() {
            if (this.buttonPerm && (JSON.stringify(this.buttonPerm) != "{}")) {
                return this.buttonPerm['sign.contractManage'] || [];
            } else {
                return false;
            }
        },
        btnList() {
            return [
                ...
                {
                    hidden: this.myButtonPrem && this.myButtonPrem.indexOf('downloadScanningCopy') != -1 ? false : true,
                    style: {},
                    type: '',
                    btnClick: this.downloadALLCopy,
                    text: this.$t('downloadALLCopy'),
                    // text: '批量下载扫描件',
                    value: '1'
                },
                [
                    {
                        ...
                        value: '1',
                        parentName: this.$t('modules_spoc_sign_web_src_views_contractmanage_contractmanage_5705')
                    },
                    {
                        hidden: this.myButtonPrem && this.myButtonPrem.indexOf('exportCt') != -1 ? false : true,
                        style: {},
                ...
                [
                    {
                        ...
                        text: this.$t('integralflow_4'),
                        value: '1',
                        parentName: this.$t('modules_spoc_sign_web_src_views_contractmanage_contractmanage_5707')
                    },
                    {
                        hidden: this.myButtonPrem && this.myButtonPrem.indexOf('exportCourse') != -1 ? false : true,
                        style: {},
                ...
                ],
                {
                    ...
                    text: this.$t('modules_spoc_sign_web_src_views_contractmanage_components_exportinovicemodal_5627'),
                    value: '1'
                }
            ];
        }
    ...
    mounted() {
        ...
        sysDict
            .batchListData({
                types: 'ht_contract_status,ht_contract_sign_type,ht_contract_category'
            })
            .then(valid.call(this))
            .then(res => {
                if (res.ok) {
                    let isEn = localStorage.getItem('local') == 'en-US'
                    if(isEn){
                        res.data.data.ht_contract_status.forEach(item=>{
                            if(item.remarks){
                    ...
                    this.categoryList = res.data.data.ht_contract_category;
                }
            })
            .catch(errors.call(this))
            .finally(() => {});
    ...
    methods: {
        ...
        downloadALLCopy(){
            ...
            if(ids.length != this.selectedRows.length){
                this.$Message.error(this.$t('downloadALLCopy_warning'));
                return;
            }
        ...
        getRejectCount() {
            let params = {
                ...
                receiptAudit:this.receiptAudit,
                phone: this.phone,
                ...
                stateGroupType:"contract management list"
            }
            htContract
        ...
        isExport(val) {
            if (val == '2') {
                ...
                this.$refs.exportModal.noShowExport(htContract.exportContract, params);
            } else {
                let _exportObj = {};
                ...
                this.$refs.exportModal.noShowExport(htContract.exportContract, _exportObj);
            }
        },
        isExportCourse(val) {
            if (val == '2') {
                ...
                this.$refs.exportModal.noShowExport(htContract.exportSelectKCB, params);
                ...
                this.$refs.exportModal.noShowExport(htContract.exportKCB, _exportObj);
        ...
        getDoAction(h, params) {
            ...
            if (this.myButtonPrem && this.myButtonPrem.indexOf('uploadSM') != -1) {
                buttonList.push({ label: params.row.isUploaded == '1' ? this.$t('modules_spoc_sign_web_src_views_contractmanage_contractmanage_5726'):this.$t('modules_spoc_sign_web_src_views_contractmanage_components_scanningcopymodal_5652'), key: 'scan' });
            }
    ...
    watch: {
        'app.currOfficeId': function(val, oldVal) {
            if (oldVal && this.$route.name == 'sign.contractManage') {
                if (this.app.currOfficeId == 'all') {
                    this.campusIds = [];
                } else {
                    this.campusIds = [this.app.currOfficeId];
        ...
        '$route.params.studentName': function(val, oldVal) {
            if (val && this.$route.name == 'sign.contractManage') {
