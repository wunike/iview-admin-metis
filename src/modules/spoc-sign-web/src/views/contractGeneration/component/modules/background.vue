			...
			<Row type="flex">
				...
				<FormItem :label="$t('modules_spoc_sign_web_src_views_contractgeneration_component_modules_background_5272')">
					<div v-if="items.itemId">
						<Input v-for="item in data.htItemList" :key="item.id" v-model="items.publicPrice" v-if="item.id==items.itemId" style="width:262px;" @on-blur="ratioChange(items,$event)" ><span slot="append">{{$t('modules_spoc_crm_web_src_views_activityenrollment_components_createevent_617')}}</span></Input>
					</div>
					<div v-else>
