...
<template>
    <Modal v-model="modalMap" :title="$t('modules_spoc_portal_modules_mapmodel_3323')" width="800" :closable="true" :mask-closable="true" class-name="mapModel vertical-center-modal" @on-visible-change="modalMapVisible">
        <div class="map_wrap" v-if="modalMap">
            <bm-control>
                <bm-auto-complete @searchcomplete="searchcomplete">
                    <div class="search">
                    <span class="label">{{$t('views_staff_components_lefttree_641')}}</span>
                    <Input element-id="suggestId" v-model="keyword" :placeholder="$t('modules_spoc_portal_modules_mapmodel_3325')" style="width:300px" />
                    </div>
                </bm-auto-complete>
                <div class="point">
                    <span class="label">{{$t('modules_spoc_portal_modules_mapmodel_3326')}}</span>
                    <Input v-model="lng" disabled style="width: 200px" />
                    <span class="label">{{$t('modules_spoc_portal_modules_mapmodel_3327')}}</span>
        ...
        <div slot="footer">
            <Button @click="modalMap = false">{{$t('classroom_allscore_53')}}</Button>
            <Button type="primary" @click="mapOk">{{$t('classroom_allscore_54')}}</Button>
        </div>
    </Modal>
</template>

<script>
import valid, { errors, sys, sysDict } from '../../../libs/request';
            ...
            function setPlace() {
                ...
                function myFun() {
                    ...
                    gc.getLocation(pp, function(rs) {
                        var addComp = rs.addressComponents;
                        console.log(rs, this.$t('modules_spoc_portal_modules_mapmodel_3330')); //地址信息
                        console.log(rs.address); //弹出当前所在地址
                        _this.keyword = rs.address;
                        _this.province = rs.addressComponents.province;
                        _this.city = rs.addressComponents.city;
