...
<template>
    <div class="take-course-container">
        <Modal class="take-course" width="680" v-model="takeCourse" :mask-closable="false" :title="$t('modules_spoc_crm_web_src_views_customer360_index_1098')">
                    ...
                    <div class="cp-course-item">
                        ...
                        <div class="new-course">
                            <div class="combination-course">
                                <Select v-model="kcbId" ref="kcbSel" clearable prefix="ios-search" filterable :placeholder="$t('modules_spoc_sign_web_src_views_contractgeneration_component_vitemcard_5449')" style="width:240px;margin-right:16px;" remote @on-query-change="kcbRemote" @on-change="addKcb" :loading="kcbLoading">
                            ...
                            <div class="new-course-box">
                                <div v-for="(items, indexs) in ncList" :key="items.id+indexs" class="new-course-item">
                                    ...
                                    <div class="nc-2">
                                        ...
                                        <Input v-model="items.endNum" style="width: 60px" @on-blur="endNumChange(items,$event,indexs)"></Input>
                                        <b>×</b>
                                        <Input disabled v-model="items.costPrice" style="width: 60px" @on-blur="endCostPriceChange(items,$event,indexs)"></Input>
                                        <b>=</b>
                                        <Input disabled v-model="items.publicPrice" style="width: 100px" @on-blur="endPublicPriceChange(items,$event,indexs)"></Input>
                                    </div>
                                    <div class="nc-3">
                                        ...
                                        <div style="width:268px;" @click.stop="tags_boxShow(items,indexs)">
                                        	...
                                        	<div class="tags_box2" v-else>
                                        		<Row type="flex" class="Row" align="middle">
                                        			<Col span="23">
                                        			<Tag>{{$t('modules_spoc_sign_web_src_views_contracttransfercenter_components_takecourse_5866', [(items.discountAmount*1).toFixed(2)])}}</Tag>
                                        			</Col>
                                        			<Col span="1">
                                    ...
                                    <div class="nc-2" v-if="items.jsonData.policyList.findIndex(v=>v.type=='relief')!=-1">
                                        ...
                                        <Input v-model="items.jsonData.policyList.find(v=>v.type=='relief').exemptionPrice" style="width: 100px" @on-blur="setDiscount(items,indexs)" ></Input>
                                    </div>
                                    <div class="nc-2">
                                        <span>{{$t('modules_spoc_sign_web_src_views_contractgeneration_component_contractinfo_5235')}}</span>
                                        <Input v-model="items.leaveNum" style="width: 100px" @on-blur="setleaveNum(items,indexs)" ></Input>
                                    </div>
                                    <div class="nc-4">
                                        <div>
                                    ...
                                    <div v-show="items.ifDiscount" class="nc-5">
                                        ...
                                        <Input :min="0" v-model="items.discount" :disabled="items.discountDisabled" style="width: 60px;" @on-blur="discountChange(items,indexs,$event)" ></Input>
                		...
                		<Row>
                		    <Col span="11">
                                <div class="counts-item">
                                    ...
                                    <Select style="width: 180px;" v-model="inCtTplId" clearable placeholder="请选择合同模板" filterable>
            ...
            <div slot="footer">
                ...
                <Button type="primary" @click="submit">{{$t('courselist_compontents_servicecontent_215')}}</Button>
            </div>
        </Modal>
    </div>
</template>
...
<script>
import valid, { errors, htContract, common, htTranslateApply, sysDict, sysUser, htContractTpl } from '../../../libs/request';
...
export default {
    ...
    methods: {
            ...
            htTranslateApply
                ...
                .then(res => {
                    if (res.ok) {
                        ...
                        this.formItem.inSellerId = data.inSellerId;
                        this.type = data.type;
        ...
        endNumChange(item, e, indexs) {
            ...
            item.policyIds = [];
            item.publicPrice = this.floatMul(num , item.costPrice);
        ...
        discountChange(item, indexs, e) {
            item.discount=Number(e.currentTarget.value)||0;
                if(String(parseFloat(Number(item.discount))).indexOf('.')!=-1){
                    this.$Modal.warning({
                        title: '课时抵扣',
                        content: `${item.name}抵扣课时必须为整数`
                    });
                }
                /* 自己抵扣不计算金额    ——by韩勇 */
        ...
        submit() {
            ...
            let params = {
                ...
                "inPayPrice": ((this.carryforward-(this.rollOut-this.residue))>=0?(this.carryforward-(this.rollOut-this.residue)):(-1*(this.carryforward-(this.rollOut-this.residue)))).toFixed(2)
            }
            params.inCtTplId = this.inCtTplId
            let inJson = [];
            let inCourseNames = [];
            let inTotalPrice = 0;
            let discountRemarks=[];
            ...
            this.ncList.forEach((v, k) => {
                    if(String(parseFloat(Number(v.discount))).indexOf('.')!=-1){
                        flag=true;
                        nameArr.push(v.name);
                    }
            ...
            console.log(params,'选课')
            if(flag){
                this.$Modal.warning({
                    title: '课时抵扣',
                    content: `${nameArr.join('、')}抵扣课时必须为整数`
                });
                return;
            }
