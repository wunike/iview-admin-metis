...
<template>
    <div class="sysTable">
        <div class="sysTable_tit">系统属性列表</div>
        <Table :columns="columns" :data="dataList" class="tbl" :draggable="true" @on-drag-drop="tableDrag">
            ...
            <template slot-scope="{ row, index }" slot="field">
                ...
                <span class="click_span">{{ row.field }}</span>
            </template>
            <template slot-scope="{ row, index }" slot="type">
                ...
                <span class="click_span" v-else>{{ row.type }}</span>
            </template>
            <!-- <template slot-scope="{ row, index }" slot="dictValue">
                ...
                <span class="click_span">{{ row.dictValue || '/' }}</span>
            </template> -->
            ...
            <template slot-scope="{ row, index }" slot="queryTypeValue">
                ...
                <span class="click_span" v-else>{{ row.queryType&&queryTypeList.find(v=>v.value==row.queryType).label || '/' }}</span>
            </template>
            ...
            <template slot-scope="{ row, index }" slot="action">
                <div class="active_box">
                    <a href="javaScript:void(0);" v-if="editIndex === index" @click="save(row)">保存</a>
    ...
    methods: {
        ...
        save(item, isCheck) {
            ...
            if (!isCheck) {
                ...
                obj.type=this.formList.find(v=>v.value==this.editjdbcType).label;
