...
<template>
    <div class="import-page-container">
        ...
        <div class="count" v-if="uploadDateOver&&$route.name!='hootie.jxStudentImport'">
            {{$t('importPage_result1before')}}<!-- 共解析数据 -->
            <span class="num" style="color: #56BC15;">{{
                retData.totalNum
            }}</span>
            {{$t('importPage_result1after')}}{{$t('importPage_result2before')}}
            <span class="num" style="color: #56BC15;">{{
                retData.successNum
            }}</span>
            {{$t('importPage_result2after')}}{{retData.overwrite>0? $t('importPage_result4before'):''}}
            <span class="num" style="color: #FF9300;">{{
                retData.overwrite>0? retData.overwrite:''
            }}</span>
            {{retData.overwrite>0? $t('importPage_result4after') :''}}{{$t('importPage_result3before')}}
            <span class="num" style="color: #E72B00;">{{
                retData.failureNum
            }}</span>
            {{$t('importPage_result3after')}}
        </div>
        <div class="count" v-if="uploadDateOver&&$route.name=='hootie.jxStudentImport'">
            <!-- 共解析数据 -->{{$t('importPage_result1before')}}
            <span class="num" style="color: #56BC15;">{{
                retData.totalNum
            }}</span>
            <!-- 条，成功导入 -->{{$t('importPage_result1after')}}{{$t('importPage_result2before')}}
            <span class="num" style="color: #56BC15;">{{
                retData.successNum
            }}</span>
            <!-- 条，无法导入的错误数据 -->{{$t('importPage_result2after')}}{{$t('importPage_result3before')}}
            <span class="num" style="color: #E72B00;">{{
                retData.failureNum
            }}</span>
            <!-- 条。 -->{{$t('importPage_result3after')}}
        </div>
        <div class="import-page-card" v-if="retData.overwrite">
            <div class="table-top">
                <div class="table-count">
                    <!-- 待确定数据 -->{{$t('importPage_result4before')}}
                    <span class="num" style="color: #FF9300;">{{
                        retData.overwrite
                    }}</span>
                ...
                <div class="table-btns">
                    <Button type="primary" style="margin-right: 16px;" @click.native.stop="coverData('cover')" :title="$t('importPage_action1title')"><!-- 覆盖数据 -->{{prevPage != 'jxStudentImport' ? $t('importPage_action1') : $t('importCoverButtonNew')}}</Button>
                    <Button type="primary" @click.native.stop="coverData('unCover')" :title="$t('importPage_action2title')"><!-- 取消 -->{{$t('importPage_action2')}}</Button>
                    <Button type="primary" style="margin-left: 10px;" v-if="isAdd" @click.native.stop="coverData('isAdd')" :title="$t('importPage_action3title')"><!-- 新增 -->{{$t('importPage_action3')}}</Button>
                </div>
            </div>
            <Table ref="selection" @on-selection-change="selectOverwriteData" :columns="columnsConflict" :data="retData.repeatList"></Table>
            <div class="page-div">
                <Page
                    :current="pageNoCover"
                    :total="countCover"
                    show-elevator
                    :page-size="pageSizeCover"
                    @on-change="pageChangeCover">
                </Page>
            </div>
        </div>
        <!-- <div class="import-page-card" id="errorCord" v-if="retData.failureNum"> -->
        <div class="import-page-card" id="errorCord" v-if="retData.allReportList.length>0">
            <div class="table-top">
    ...
    export default {
        ...
        methods: {
            ...
            doupload(response) {
                ...
                if (data.overwrite + data.failureNum == 0) {
                    // 导入成功，弹窗提示
                    this.onEnd(data)
                } else {
                    // 有异常数据
                    ...
                    //  if(this.$route.name=='hootie.courseListImport'){
                    ...
                    // }
                    // else{
                    //     this.retData.allReportList = data.allReportList
                    //         ? data.resultList
                    //         : []
                    //     this.retData.repeatList = data.repeatList
                    //         ? data.repeatList
                    //         : []
                    // }
                    this.pageNo = 1
                        ...
                        if (str == 'cover' || str == 'isAdd') {
                            if(this.$route.name =='hootie.jxStudentImport'){
                                ...
                                if(res.data.data.length){
                                    ...
                                    this.$Message.success('所有学员班课新增完毕！')
                                }
                            } else {
                                this.retData.successNum += n
                            }
            ...
            downloadTemplate() {
                // 下载模板
                // console.log(this.templateLists.download());
                // window.open(this.templateLists.download());
                let $eleForm = document.createElement('form')
                    if(this.$route.query.officeId){
                        if(!this.officeId){
                            // this.$Message.error('请选择所属校区')
                            this.$Message.error(this.$t('importPage_warning2'))
                            return false
                        } else{
                        //    templateLists.checkTemplateData().then()
				//判断校区是否有模板
                            this.templateLists
                                .checkTemplateData({officeId:this.officeId})
                                .then(valid.call(this))
                                .then(res => {
                                    if(res.ok) {
                                        $eleForm.method = 'post';
                                        $eleForm.action = this.templateLists.download('?officeId='+this.officeId);
                        ...
                        // $eleForm.methods = 'get'
                        // $eleForm.action = this.templateLists.download()
                        // document.body.appendChild($eleForm)
                        // //提交表单，实现下载
                        // $eleForm.submit()
                        this.updateLoadingStatus({isLoading:true});
